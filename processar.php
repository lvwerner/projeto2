...
$pressao_arterial1 = isset($_POST["pressao-arteral1"]) ? $_POST["pressao-arteral1"] : "";
$pressao_arterial2 = isset($_POST["pressao-arteral2"]) ? $_POST["pressao-arteral2"] : "";
...
$sql = "INSERT INTO dadospaciente (oq_aconteceu, aconteceu_outras_vezes, tempo_aconteceu, possui_problema_saude,
quais_doencas, faz_uso_medicacao, horario_ultima_medicacao, quais_medicacoes, alergico_alguma_coisa, especificacao_alergia,
ingeriu_limento_liquido, horas_ingeriu, data, sexo, nome_hospital, nome_paciente, idade, fone, rg_cpf_paciente, nome_acompanhante,
idade_acompanhante, local_ocorrencia, periodo_gestacao, faz_uso_medicacao, nome_medico, complicacoes, primeiro_filho, quantidade_filhos,
inicio_contracoes, duracao_contracoes, intervalo_contracoes, pressao_evacuar, ruptura_bolsa, inspecao_visual, hora_nascimento, sexo_bebe,
nome_bebe, causas_animais, causas_meio_transporte, causas_desmoronamento, causas_emergencia_medica, 
...
causas_trabalho, causas_transferencia, outras_causas, spontanea, comando_verbal, estimulo_doloroso, nenhuma,
orientado, palavras_frase_apropriadas, conflito, palavras_inapropriadas, palavras_inapropriadas2,
choro_persistente, palavras_incompreensiveis, sons_incompreensiveis, nenhuma_resposta_verbal,
obedece_comandos, obedece_prontamente, localiza_dor, localiza_dor_ou_estimulo_tatil,
movimento_de_retirada, retirada_do_segmento_estimulado, flexao_anormal, flexao_anormal2,
extensao_anormal, extensao_anormal2, nenhuma_resposta_motora, ausencia,
total_gcs_1, total_gcs_2, pressao_arterial1, pressao_arterial2, pulso, respiracao,
pulsacao1, pulsacao2, temperatura, perfusao, anormal, normal, psiquiatrico, respiratorio, diabetes,
obstetrico, transporte, prob_outros) 
VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?,?, ?, ?, ?, ?, ?)";

$stmt = $conexao->prepare($sql);
$stmt->bind_param("ssssssssssssssssissiissssssisssssssssiiiiiiiiiiiiiiiiiiiisiiiiiiiiiiiiiiiiiiiiiiiiissssssssssssssss",
$oq_aconteceu, $aconteceu_outras_vezes, $tempo_aconteceu, $possui_problema_saude, 
$quais_doencas, $faz_uso_medicacao, $horario_ultima_medicacao, $quais_medicacoes, 
$alergico_alguma_coisa, $especificacao_alergia, $ingeriu_limento_liquido, $horas_ingeriu, 
$data, $sexo, $nome_hospital, $nome_paciente, $idade, $fone, $rg_cpf_paciente, $nome_acompanhante, 
$idade_acompanhante, $local_ocorrencia, $periodo_gestacao, $faz_uso_medicacao, $nome_medico, 
$complicacoes, $primeiro_filho, $quantidade_filhos, $inicio_contracoes, $duracao_contracoes, 
$intervalo_contracoes, $pressao_evacuar, $ruptura_bolsa, $inspecao_visual, $hora_nascimento, 
$sexo_bebe, $nome_bebe, $causas_animais, $causas_meio_transporte, $causas_desmoronamento, 
$causas_emergencia_medica, $causas_queda_altura_2m, $causas_tentativa_suicidio, 
$causas_queda_propria_altura, $causas_afogamento, $causas_agressao, $causas_atropelamento, 
$causas_choque_eletrico, $causas_desabamento, $causas_domestico, $causas_esportivo, 
$causas_intoxicacao, $causas_queda_bicicleta, $causas_queda_moto, $causas_queda_nivel_2m,
$causas_trabalho, $causas_transferencia, $outras_causas, $espontanea, $comando_verbal, $estimulo_doloroso, $nenhuma,
$orientado, $palavras_frase_apropriadas, $conflito, $palavras_inapropriadas, $palavras_inapropriadas2,
$choro_persistente, $palavras_incompreensiveis, $sons_incompreensiveis, $nenhuma_resposta_verbal,
$obedece_comandos, $obedece_prontamente, $localiza_dor, $localiza_dor_ou_estimulo_tatil,
$movimento_de_retirada, $retirada_do_segmento_estimulado, $flexao_anormal, $flexao_anormal2,
$extensao_anormal, $extensao_anormal2, $nenhuma_resposta_motora, $ausencia,
$total_gcs_1, $total_gcs_2, $pressao_arterial1, $pressao_arterial2, $pulso, $respiracao, $pulsacao1,
$pulsacao2, $temperatura, $perfusao, $anormal, $normal, $psiquiatrico, $respiratorio, $diabetes,
$obstetrico, $transporte, $prob_outros);
